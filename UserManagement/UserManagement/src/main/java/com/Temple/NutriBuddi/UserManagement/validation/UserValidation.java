...
public class UserValidation {
	...
	public ValidationResponse validateNewEmail(String email, UserRepository userRepository) {
		if (!email.equals("")) {
	...
	public ValidationResponse validateUserName(User user, String password, String password2){
		if (!password.equals("")) {
			if (password.equals(password2)) {
                user.setPassword(password);
				return new ValidationResponse("Okay", HttpStatus.OK);
			} else {
				return new ValidationResponse("Passwords must match", HttpStatus.NOT_ACCEPTABLE);
			}
		} else {
			return new ValidationResponse("Password required", HttpStatus.NOT_ACCEPTABLE);
		}
	}
}
